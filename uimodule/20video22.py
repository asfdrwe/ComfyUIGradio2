...
def generate(unethigh, clip, audioencoder, vae, image, audio, positive, width, height, seed, progress = gr.Progress()):
    if seed == '-1':
        seed = random.randint(0, sys.maxsize)
    else:
        seed = int(seed)
    prompt_file = open('workflow/video22.json', 'r')
...
def create(addr):
    ...
    server_address = addr
    print('SERVER: ' + server_address)
    with gr.Tab('開始画像と音声と文章から動画生成(Wan2.2)(15秒弱)'):
        with gr.Row():
            with gr.Column():
                unet_list = comfyutils.get_unetgguf(server_address)
                print(unet_list)
